				...
				<ul class="parentList">
					...
					<li class="level_PJY">
						...
						<ul>
							...
							<li id="category-highlight" type="category" op="eq">创业创新</li>
						</ul>
						<div class="clear"></div>
					</li>
					<li class="depart_PJY">
				...
				<div class="object_PJY">
					<div >
						...
						<ul class="indexList_PJY policy_list">
							...
							<li>
								<a href="/policy?uid=${user.userId}&aid=${p.id}" title="${p.title}" target="_self">
...
$(document).ready(function(){
    ...
    if(decodeURI(location.search).indexOf("创业")>0) {
		$("#category-highlight")。trigger("click");
	}
});
