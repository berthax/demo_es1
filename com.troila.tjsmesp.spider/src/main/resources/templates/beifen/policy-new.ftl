...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/policy/assets/js/kindeditor/plugins/code/prettify.css" />
    <script src="/policy/assets/js/kindeditor/kindeditor.js"></script>
    <script src="/policy/assets/js/kindeditor/lang/zh_CN.js"></script>
	<script charset="utf-8" src="/policy/assets/js/kindeditor/plugins/code/prettify.js"></script>
	...
	.po_ly_li li label.error{position:absolute;right:-70px;text-align:left;width:60px}
 	...
 	.ke-container .ke-container-default{width:740px !important;}
	</style>
</head>
...
<div id="header">
		...
		<div class="admMmainR">
			...
			<div class="bgf">
				<ul class="po_ly_li clearfix">
					...
					<li class="clearfix">
						...
						<div class="adminSelect selW350">
                            <select name="category" id="category">
                            	<option <#if policy?? && policy.category?? && policy.category=="不限">selected</#if>>不限</option>
								<option <#if policy?? && policy.category?? && policy.category=="综合政策">selected</#if>>综合政策</option>
								<option <#if policy?? && policy.category?? && (policy.category=="金融税收" || policy.category=="财税金融")>selected</#if>>财税金融</option>
								<option <#if policy?? && policy.category?? && policy.category=="创业创新">selected</#if>>创业创新</option>
								<option <#if policy?? && policy.category?? && policy.category=="人才引进">selected</#if>>人才引进</option>
	                            <option <#if policy?? && policy.category?? && policy.category=="知识产权">selected</#if>>知识产权</option>
	                            <option <#if policy?? && policy.category?? && policy.category=="招商引资">selected</#if>>招商引资</option>
					...
					<!-- 起始日期改为发布日期，对应2019-1-8政策提出的需求，2019-1-9-宣修改 -->
					<li class="clearfix"><label for="gmtStart">发布日期：</label><input id="gmtStart" name="gmtStart" type="text" class="w350 datepicker" readonly value="<#if policy?? && policy.gmtStart??>${policy.gmtStart?date}</#if>"></li>
					<!-- 截至日期暂时隐藏，对应2019-1-8政策提出的需求，2019-1-9-宣修改 -->
					...
					<!-- 扶持行业隐藏，对应2019-1-8政策提出的需求，2019-1-9-宣修改-->
					<li class="clearfix" style="display:none">
					...
					<!-- 转载来源隐藏，对应2019-1-8政策提出的需求，2019-1-9-宣修改-->
					<li class="clearfix" style="display:none"><label for="fromSite">转载来源：</label><input id="fromSite" name="fromSite" type="text" class="w350" value="<#if policy??>${policy.fromSite}</#if>"></li>
					<!-- 转载链接改为原文链接，对应2010-1-8政策提出的需求，2018-1-9-宣修改-->
					<li class="clearfix"><label for="fromLink">原文链接：</label><input id="fromLink" name="fromLink" type="text" class="w350" value="<#if policy??>${policy.fromLink}</#if>"></li>
					<!-- 政策图片去掉，对应2019-1-8政策提出的需求，2019-1-9-宣修改-->
					<!-- <li class="clearfix">
					...
					<!-- 核批人去掉，2019-1-9-宣修改-->
					<#-- <li class="clearfix" style="display: none"><label for="supervisor">&nbsp;&nbsp;核批人：</label><input id="supervisor" name="supervisor" type="text" class="w350" value="<#if policy??>${policy.supervisor}</#if>"></li> -->
					<!-- 关键字隐藏 -->
					<li class="clearfix" style="display: none">
						<label for="">关键字：</label>
					...
					<!-- 摘要去掉，对应2019-1-8政策提出的需求，2019-1-9-宣修改-->
					...
					<!-- chop发布摘编，tag=="edit"&&policy??&&policy.type=1为编辑摘编   -->
					<#if tag == "chop"|| (tag=="edit"&&policy??&&policy.type=1)>
	...
	<div class="adm_footer clear">
		...
		<p>版权所有：天津市中小企业生产力促进中心 |津ICP备17002884<br>技术支持：天津市卓朗科技发展有限公司</p>	
	</div>
	<!--放大图片-->
	<div class="pic_pop" >
		<a class="picclose"></a>
...
/* 	        KindEditor.ready(function(K) {
	        ...
	        KindEditor.ready(function(K) {
	        	...
	        	$(".editor").each(function(i, ele) {
	        		...
	        		var height = $editor.css("height");
	        		var id = $editor.attr("id");
//	        		window.editor[id] = K.create('textarea[name="'+id+'"]', {
	        		window.editor[id] = K.create("#" + id,{
	        			cssPath : '/policy/assets/js/kindeditor/plugins/code/prettify.css',
						height:"359px",
//						height:height,
						width:"970px",
		                height: height,
		        ...
		        ,'lineheight','clearhtml','table','hr','emoticons','image', 'baidumap','link','unlink',
		        'formatblock', 'fontname', 'fontsize', 'forecolor', 'hilitecolor', 'bold', 'italic', 'underline', 'preview','wordpaste'
						],
						uploadJson : '/policy/attach/kindeditor',
						fileManagerJson : '/policy/attach/kindeditor',
		...
		function getEntity() {
			...
			//运营中心用户发布政策可以不输入文号，枢纽平台的必须输入文号
			<#if from??>
				<#if from==2>
				    //枢纽平台的用户发布政策必须输入文号
					if(!referenceNumber) {
						layer.msg("请输入文号...", {icon:3, time: 2000});
						throw "invalid";
					}
				</#if>
			</#if>
			entity.industry = $("#industry").val().join();   //没有扶持产业
			entity.area = $("#area").val().join();
			<#if tag=="edit">
			entity.id = ${policy.id?c};
			entity.parentId = ${policy.parentId?c}
			<#else>
			entity.parentId = <#if policy??>${policy.id?c}<#else>-1</#if>
			</#if>
			...
			//起始日期改为发布日期，为必填项
			if(!date) {
				layer.msg("发布日期为必填项", {icon:3, time: 2000});
				throw "invalid";
			}
...
//			var field0 = "<h3 class='articleTit'>摘要</h3><div class='articleTxt'>" + window.editor["policyAbstract"].html()+"</div>";
		...
		$(function(){
			...
			KindEditor.ready(function(){
				...
				//window.editor["field1"].html(pieces[1].innerHTML);  //申报条件
				//window.editor["field2"].html(pieces[2].innerHTML);  //扶持重点
				//window.editor["field3"].html(pieces[3].innerHTML);  //支持方式及金额
				//window.editor["field4"].html(pieces[4].innerHTML);  //申报材料
				//window.editor["contact"].html(pieces[5].innerHTML);
				
				window.editor["field1"].html(pieces[pieces.length-5].innerHTML);  //申报条件
				window.editor["field2"].html(pieces[pieces.length-4].innerHTML);  //扶持重点
				window.editor["field3"].html(pieces[pieces.length-3].innerHTML);  //支持方式及金额
				window.editor["field4"].html(pieces[pieces.length-2].innerHTML);  //申报材料
				window.editor["contact"].html(pieces[pieces.length-1].innerHTML); //联系人
				<#else>
				//去掉有关政策摘要的字段相关内容
				//window.editor["policyAbstract"].html(pieces[0].innerHTML);
				//window.editor["content"].html(pieces[1].innerHTML);
				//window.editor["contact"].html(pieces[2].innerHTML);
				//之前的政策可能是有摘要的，这里需要区分一下，最后两个一个是联系人，一个是内容
				window.editor["content"].html(pieces[pieces.length-2].innerHTML);
				window.editor["contact"].html(pieces[pieces.length-1].innerHTML);
				</#if>
			})
			</#if>
		});
