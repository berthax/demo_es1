...
@Component
public class JinghaiIndustrialClustersNewsPageProcessor implements PageProcessor{
	...
	public void process(Page page) {
		if(page.getUrl().regex(LIST_URL).match()) {
			...
			if(null != articleList && articleList.size() > 0) {
				//	查看已经爬取过的链接记录
				pastCrawledUrls = newsProcessorService.getCrawledUrls(SpiderModuleEnum.JINGHAI_INDUSTRIAL_CLUSTERS_NEWS.getIndex());
			...
			String title = page.getHtml().xpath("//div[@class='title']/h4/tidyText()").toString();
...
//			spider.setContent(content);		
			...
			spider.setSpiderModule(SpiderModuleEnum.JINGHAI_INDUSTRIAL_CLUSTERS_NEWS.getIndex());
			
			//获取内容中所有的链接
			List<String> urls =  page.getHtml().xpath("//div[@class='content']").links().all();
